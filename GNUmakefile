...
rootfs.img: build-rootfs.sh
	./build-rootfs.sh

$(IMAGE_NAME).iso: limine/limine kernel rootfs.img
	rm -rf iso_root
...
clean:
	$(MAKE) -C kernel clean
	rm -rf iso_root $(IMAGE_NAME).iso $(IMAGE_NAME).hdd rootfs.img
...
.PHONY: format
format:
	@find ./kernel -iname '*.h' -o -iname '*.c' | xargs clang-format -i --style=LLVM
